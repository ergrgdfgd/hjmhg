...
st.write(f"Текущий баланс: {balance:.2f} рублей")

# Кнопка для открытия кейса
if st.button("Открыть кейс"):
    if balance >= case_cost or balance > -100:  # Проверяем, хватает ли баланса, чтобы открыть кейс
        balance -= case_cost  # Уменьшаем баланс на стоимость кейса
        result = open_case()
        
        if result:
            st.write(f"Вы выиграли: {result[0]} - Цена: {result[1]} рублей")
            balance += result[1]  # Прибавляем стоимость предмета к балансу
        else:
            st.write("Попробуйте снова!")
        
        st.write(f"Текущий баланс: {balance:.2f} рублей")
    else:
        st.write("Недостаточно средств для открытия кейса!")
